...
Apify.main(async () => {
  ...
  // timestamp (if want to pause after some time periodically)
  ...
  while (infiniteRounds || round <= maxRounds) {
    ...
    const crawler = new Apify.PuppeteerCrawler({
      ...
      handlePageFunction: async ({ request, page, response }) => {
        ...
        // Take a break for slightly more than a day if been running for about 3 days.
        var time1 = performance.now();
        var timediff = time1/1000.0 - time0/1000.0
        if (timediff > 257000) {
          await delay(86411111)
        }
